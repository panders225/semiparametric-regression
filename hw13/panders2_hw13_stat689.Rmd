...
names(colon) <- tolower(names(colon))
str(colon)
```
...
# Question 3
Describe the results from the above regression.
...
It appears that smoothing the terms is necessary.

# Question 7
Plot the smooths, their pointwise CI's, and the Cox residuals.

```{r}
# plot the smoothed terms first
mgcv::plot.gam(cox_mod_two, shade=T, shade.col='dodger blue')
```
...
set.seed(1738)
cases <- ceiling(runif(4, 0, nrow(colon)))
